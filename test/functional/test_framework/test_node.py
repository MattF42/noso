...
"""Class for nosord node under test"""
...
class TestNode():
    """A class for representing a nosord node under test.
    ...
    def __del__(self):
        # Ensure that we don't leave any nosord processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time nosord is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by nosord, and
        ...
        self.running = True
        self.log.debug("nosord started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the nosord process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'nosord exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; nosord is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to nosord after {}s".format(self.rpc_timeout))
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to nosord
        expected_msg: regex that stderr should match when nosord fails

        Will throw if nosord starts without an error.
        Will throw if an expected_msg is provided and it does not match nosord's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'nosord exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'nosord should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to nosor-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with nosor-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run nosor-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running nosor-cli {}".format(p_args[2:]))
