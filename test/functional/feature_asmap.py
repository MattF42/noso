...
Verify node behaviour and debug log when launching nosord in these cases:

1. `nosord` with no -asmap arg, using /16 prefix for IP bucketing

2. `nosord -asmap=<absolute path>`, using the unit test skeleton asmap

3. `nosord -asmap=<relative path>`, using the unit test skeleton asmap

4. `nosord -asmap/-asmap=` with no file specified, using the default asmap

5. `nosord -asmap` restart with an addrman containing new and tried entries

6. `nosord -asmap` with no file specified and a missing default asmap file

7. `nosord -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(BitcoinTestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test nosord with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_asmap_with_absolute_path(self):
        self.log.info('Test nosord -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test nosord -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_default_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info(f'Test nosord {arg} (using default map file)')
            self.stop_node(0)
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info("Test nosord -asmap restart with addrman containing new and tried entries")
        self.stop_node(0)
        shutil.copyfile(self.asmap_raw, self.default_asmap)
    ...
    def test_default_asmap_with_missing_file(self):
        self.log.info('Test nosord -asmap with missing default map file')
        self.stop_node(0)
        msg = f"Error: Could not find asmap file \"{self.default_asmap}\""
        self.node.assert_start_raises_init_error(extra_args=['-asmap'], expected_msg=msg)

    def test_empty_asmap(self):
        self.log.info('Test nosord -asmap with empty map file')
        self.stop_node(0)
